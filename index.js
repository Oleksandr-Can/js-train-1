...
function isEvenNumber(num) {
  if (num % 2 === 0) {// Перевірити, чи число num є парним
  return true;}
   return false;// Повернути результат перевірки (true або false) як результат функції
}
...
function calculateSum(n) {
  ...
  for (let i = 0; i <= n; i++) {// Запустити цикл для змінної i від 1 до n
...
function isPrimeNumber(num) {
  ...
  for (let i = 2; i <= num - 1; i++) {// Запускаємо цикл для змінної i від 2 до num-1
   if (num % i === 0) {// Перевіряємо, чи num ділиться націло на поточне значення 
...
function checkDataType(value) {
  ...
  if (typeof value === 'number') {// Виводимо у консоль повідомлення, яке містить значення value та його тип
  return 'number'}
  else if (typeof value === 'string') {// Повертаємо тип значення value
    return 'string'}
...
function findMax(num1, num2) {
  ...
  return num1;}
  else if (num1 === num2) {// Якщо num1 більше num2, то num1 є максимальним, тому повертаємо num1
  return "equaly";}
...
function getUserStatus(age) {
  ...
  else if (age >= 18 && age < 65) {// Якщо age більше або дорівнює 18 і менше 65, то користувач є "Дорослим"
    return "Дорослий";}
    else {// В іншому випадку (якщо age більше або дорівнює 65), то користувач є "Пенсіонером"
      return "Пенсіонер";}
}
...
function getDayOfWeek(dayNumber) {
  ...
  switch (dayNumber) {
    case 1: return "Понеділок"; // Якщо dayNumber = 1, то day = "Понеділок"
    case 2: return "Вівторок";// Якщо dayNumber = 2, то day = "Вівторок"
    case 3: return "Середа";// Якщо dayNumber = 3, то day = "Середа"
    case 4: return "Четвер";// Якщо dayNumber = 4, то day = "Четвер"
    case 5: return "П'ятниця";// Якщо dayNumber = 5, то day = "П'ятниця"
    case 6: return "Субота";// Якщо dayNumber = 6, то day = "Субота"
    case 7: return "Неділя";// Якщо dayNumber = 7, то day = "Неділя"
    
  default: return "Невірний номер дня";}// Якщо значення dayNumber не співпадає з жодним з випадків, то day = "Невірний номер дня"
...
function getVariableType(variable) {
  // if (typeof variable === "number") {// Отримуємо тип змінної variable за допомогою оператора typeof
  // Встановлюємо відповідне повідомлення залежно від отриманого типу
  switch (typeof variable) {
    case "number": return "Це число";
    case "string": return "Це рядок";
    case "boolean": return "Це булеве значення";
    default: return "Це інший тип";

  }
//  return "Це число"; }// Якщо тип змінної - number, то повідомлення = "Це число"
//   else if (typeof variable === "string") {
//     return  "Це рядок"; }// Якщо тип змінної - string, то повідомлення = "Це рядок"
//     else if (typeof variable === "boolean") {
//       return  "Це булеве значення";} // Якщо тип змінної - boolean, то повідомлення = "Це булеве значення"
//       else {
//         return  "Це інший тип";}// В іншому випадку (якщо тип змінної не входить у попередні категорії), то повідомлення = "Це інший тип"
...
function makeAdder(x) {
  return (y) => {// Вертаємо нову функцію, яка приймає `y`
...
function divide(x) {
  return (y) => { // Повертаємо нову функцію, яка приймає `y`
...
function memoizedArea() {
  let prevSide = {};// Ініціалізуємо примітивні змінні для зберігання попереднього аргументу та результату
  return (side) => {// Повертаємо функцію яка приймає аргумент side - сторону квадрата
  if (side in prevSide){  // Перевіряємо, чи є введена сторона такою ж, як і в попередньому виклику
  console.log("Fetching from cache");// Виводимо повідомлення Fetching from cache
  return prevSide[side];}// Повертаємо попереднє значення
  ...
  let s = side **2;// Розраховуємо площу
  prevSide[side] = s;// prevSide присвоюємо сторону квадрата
  let prevResult = s;// prevResult просвоюємо площу квадрата
  return prevResult;} // Повертаємо площу
  }
}

let squareArea = memoizedArea();//Створюємо змінну squareArea якій присвоємо результат виконання функції memoizedArea
...
function compose(func1, func2) {
  return (n) => {// Повертаємо нову функцію, яка приймає аргумент n
  return double(increment(n));}// Повертаємо func2, в яку передаємо func1 з аргументом n
}

let createCompose = compose();// Створюємо змінну createCompose якій присвоємо результат виконання функції compose з аргументами increment та double
